...
const TeamCard = () => {
    ...
    useEffect(() => {
        fetch(`${backendHost}/api/v1/streams?page=0&size=150`, {
            ...
            .then((data) => {
                if (data?.content) {
                    const filteredStreams = data.content.filter(stream => stream.active === true);
    ...
    const validateForm = () => {
        ...
        if (formData.streamId && selectedMarkets.length > 0) {
            if (!checkNtiMarketsMatchWithStream(formData.streamId, selectedMarkets.map(m => m.id))) {
    ...
    const handleCreate = async () => {
        ...
        try {
            ...
            navigate(`/teamcard/${data.id}`, {
                state: { streamId: formData.streamId }
            });
        } catch (error) {
