...
describe('MobileHeader Component', () => {
  ...
  test('renders header with logo and "Track Me" text', async () => {
    await act(async () => {
      render(
        <MemoryRouter initialEntries={['/team-card/42']}>
          <Routes>
    ...
    expect(document.querySelector('.Stream-header-logo')).toBeInTheDocument();
  });
  ...
  test('navigates to profile when "Личный кабинет" is clicked', async () => {
    await act(async () => {
      render(
        <MemoryRouter initialEntries={['/team-card/42']}>
          <Routes>
  ...
  test('navigates to team-cards when "Главный экран" is clicked (TRACKER role)', async () => {
    redux.useSelector.mockImplementation(() => ({
      user: { username: 'testUser', roles: ['TRACKER'] }
    }));

    Storage.prototype.getItem = jest.fn(() =>
      JSON.stringify({ username: 'testUser', roles: ['TRACKER'] })
    );
    ...
    fireEvent.click(screen.getByRole('button', { name: '' })); // open menu
    fireEvent.click(screen.getByText('Главный экран'));

    expect(mockedNavigate).toHaveBeenCalledWith('/team-cards');
  });

  test('navigates to streams when "Главный экран" is clicked (ADMIN role)', async () => {
    await act(async () => {
      render(
        <MemoryRouter initialEntries={['/team-card/42']}>
          <Routes>
...
test('menu items are keyboard accessible', async () => {
  ...
  test('hamburger icon animates on menu toggle', async () => {
    ...
    const hamburger = screen.getByRole('button', { name: '' }).querySelector('.hamburger');
