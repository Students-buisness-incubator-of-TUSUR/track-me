...
function ProfilePage() {
  ...
  const validateForm = () => {
    ...
    // Для проверки телефона убираем все нецифровые символы
  ...
  const handlePhoneChange = (e) => {
    ...
    // Если ничего не введено, сбрасываем поле
    if (!digits) {
    ...
    // Форматирование в вид: +7 (xxx) xxx-xx-xx
    let formatted = "+7";
    if (digits.length > 1) {
  ...
  return (
    <div className="login-container">
      <div className="profile-container">
        ...
        {error && <div className="error-message42">{error}</div>}
        {!isEditing ? (
          <button
