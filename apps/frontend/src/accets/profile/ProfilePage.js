...
function ProfilePage() {
  ...
  useEffect(() => {
    ...
    fetch(window.env.PORT+"/api/v1/users/current/info", {
      method: "GET",
      headers: {
  ...
  useEffect(() => {
    ...
    fetch(window.env.PORT+`/api/v1/users/${userData.telegramId}/photo`, {
      method: "GET",
      headers: {
        Authorization: `Bearer ${token}`,
      },
  ...
  const handleSaveClick = () => {
    ...
    fetch(window.env.PORT+"/api/v1/users/current/update", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
