//NOSONAR
import React, { useState, useEffect } from 'react'; // NOSONAR
...
import { getCsrfConfigForFetch } from "../../utils/csrf-utils"; // NOSONAR

export default function EditStream() { // NOSONAR
  const { id } = useParams(); // NOSONAR
  const navigate = useNavigate(); // NOSONAR
  const [showDeleteConfirm, setShowDeleteConfirm] = useState(false); // NOSONAR
  const [showTeamsWarning, setShowTeamsWarning] = useState(false); // NOSONAR
  const [attachedTeams, setAttachedTeams] = useState([]); // NOSONAR
  const [allTeamCards, setAllTeamCards] = useState([]); // NOSONAR
  const backendHost = (process.env.REACT_APP_BACKEND_URI || 'http://localhost:8080') + '/backend'; // NOSONAR

  // Загрузка всех команд для проверки привязок // NOSONAR
  useEffect(() => { // NOSONAR
    const fetchAllTeams = async () => { // NOSONAR
      try { // NOSONAR
        const endpoint = `${backendHost}/api/v1/admin/team-cards?page=0&size=1000`; // NOSONAR
        const payload = { filters: [] }; // NOSONAR
        
        const response = await fetch(endpoint, { // NOSONAR
          method: "POST", // NOSONAR
          headers: { // NOSONAR
