...
@Builder
@Schema(description = "Запись отчета о встречах для трекера (без колонки трекера)")
public record TrackerMeetingReportRecordDto(
        @Schema(description = "ID команды")
        UUID teamId,

        @Schema(description = "Название команды, для которой была встреча")
        String teamName,
