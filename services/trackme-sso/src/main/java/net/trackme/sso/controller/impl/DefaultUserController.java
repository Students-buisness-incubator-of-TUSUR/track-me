...
/**
 * Реализация контроллера для управления пользователями.
 * Обрабатывает запросы на включение, отключение, удаление пользователей
 * и получение информации о них.
 */
@Slf4j
@RestController
...
public class DefaultUserController implements UserController {
    ...
    @Override
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<List<Map<String, String>>> getUserTeams(
            String username) {
        log.info("Getting teams for user: {}", username);
        List<Map<String, String>> teams =
                userService.getUserTeams(username);
    ...
    @Override
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<PagedModel<UserDto>> getTrackers(
    ...
    public ResponseEntity<PagedModel<UserDto>> getAdmins(
            FilterRequest filterRequest,
            Pageable pageable) {
    ...
    /**
     * Обрабатывает исключение при ошибке переназначения команды.
     *
     * @param e исключение TeamReassignmentException
     * @return ответ с ошибкой и статусом 409 Conflict
     */
